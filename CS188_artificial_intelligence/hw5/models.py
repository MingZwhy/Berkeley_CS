...
class PerceptronModel(object):
    ...
    def train(self, dataset):
        ...
        """
        "*** YOUR CODE HERE ***"
        flag = 1 # flag = 1 means the dataset is still changing
        batch_size = 1
        while flag:
            flag = 0
...
class RegressionModel(object):
    ...
    def run(self, x):
        ...
        """
        "*** YOUR CODE HERE ***"
        h1 = nn.AddBias(nn.Linear(x,self.w0) , self.b0)
        y_predicted = nn.AddBias( nn.Linear(nn.ReLU(h1),self.w1) , self.b1 )
        return y_predicted
    ...
    def train(self, dataset):
        ...
        while True:
            for x,y in dataset.iterate_once(self.batch_size):
              ...
              self.b0.update(gradient[1],-self.learning_rate)
              self.w1.update(gradient[2],-self.learning_rate)
...
class LanguageIDModel(object):
    ...
    def __init__(self):
        ...
        self.wf = nn.Parameter(hidden_size, 5)
    ...
    def run(self, xs):
        ...
        for i in range(1, len(xs)):
            h = nn.ReLU( nn.AddBias( nn.Add( nn.Linear(xs[i],self.wx) , nn.Linear(h,self.wh) ) , self.b ) )
        result = nn.AddBias(nn.Linear(h,self.wf), self.bf)
        return result
    ...
    def train(self, dataset):
        ...
        while True:
          for x,y in dataset.iterate_once(self.batch_size):
            loss = self.get_loss(x,y)
            gradient = nn.gradients(loss, [self.wx,self.b,self.wh,self.wf,self.bf])
            self.wx.update(gradient[0],-self.learning_rate)
            self.b.update(gradient[1],-self.learning_rate)
            self.wh.update(gradient[2],-self.learning_rate)
            self.wf.update(gradient[3],-self.learning_rate)
            self.bf.update(gradient[4],-self.learning_rate)
