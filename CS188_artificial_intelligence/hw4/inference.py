...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variableDomainsDict[OBS0] = list(range(X_RANGE + Y_RANGE + MAX_NOISE - 1)) #non-negative and |obs - true| <= MAX_NOISE
    variableDomainsDict[OBS1] = list(range(X_RANGE + Y_RANGE + MAX_NOISE - 1)) #non-negative and |obs - true| <= MAX_NOISE
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        #raiseNotDefined()
        # Select factors with given evidence
        FactorsWithEvidence = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        # Start vatiable elimination in order
        for variable in eliminationOrder:
            FactorsWithEvidence,FactorJoined = joinFactorsByVariable(FactorsWithEvidence,variable)
            if len(FactorJoined.unconditionedVariables()) == 1:
                continue
            FactorJoined = eliminate(FactorJoined,variable)
            FactorsWithEvidence.append(FactorJoined)
        
        final_factor = normalize(joinFactors(FactorsWithEvidence))
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        summation = self.total()
        flag = 0 # accumulate the probability of every key
        if summation == 0:
            return None
        else:
            for key,value in self.items():
                flag += value
                if rand_seed < flag/summation:
                    return key
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if ghostPosition == jailPosition:
            ...
            if noisyDistance == None:
                ...
                prob = busters.getObservationProbability(noisyDistance,true_dist)
            return prob
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        "*** YOUR CODE HERE ***"
        #raiseNotDefined()
        # Known P(NoisyDis|PacmanPosition,GhostPosition) => P(NpoisyDis,PacmanPosition,GhostPosition)
        PacmanPosition = gameState.getPacmanPosition()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        "*** YOUR CODE HERE ***"
        #raiseNotDefined()
        # newPosDist = self.getPositionDistribution(gameState, oldPos) = P(Xt+1|Xt)
        # Equation: P(Xt+1) = P(Xt+1|Xt)*P(Xt) Markov Chain
        # First, we need a Dict to store newPosDist for every oldPos to reduce calls of func self.getPositionDistribution
        newPosDist_List = {}
        for oldPos in self.allPositions:
            newPosDist = self.getPositionDistribution(gameState, oldPos)
            newPosDist_List[oldPos] = newPosDist
        
        newbliefs = DiscreteDistribution()
        # Second, calculate bliefs
        for newPos in self.allPositions:
            newProb = 0.0
            for oldPos in newPosDist_List.keys():
                newProb += newPosDist_List[oldPos][newPos] * self.beliefs[oldPos]
            newbliefs[newPos] = newProb
        
        self.beliefs = newbliefs
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        for position in self.legalPositions:
            for i in range(particles_per_pos):
    ...
    def elapseTime(self, gameState):
        ...
        NewParticles = []
        for particle in self.particles:
            # Distribution of newparticle's position of every particle
