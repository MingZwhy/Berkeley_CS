...
def rmsprop(w, dw, config=None):
    ...
    alpha, beta, ceita = config['learning_rate'] , config['decay_rate'] , config['epsilon']
    s = beta * config['cache'] + (1 - beta) * (dw ** 2)
    next_w = w - (alpha / (np.sqrt(s) + ceita)) * dw
...
def adam(w, dw, config=None):
    ...
    t = config['t']
    alpha, ceita = config['learning_rate'], config['epsilon']
    ...
    next_w = w - (alpha * m_hat) / (np.sqrt(v_hat) + ceita)
