...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        batch_z = (x - batch_mean) / np.sqrt(batch_var + eps)
        
        out = batch_z * gamma + beta
        ...
        # beta and mean are useless for backward compute gradient
        
        cache = (gamma, x, batch_var, batch_z, eps)
        ...
        # the out variable.                                                         #
        #############################################################################
        batch_z = (x - running_mean) / np.sqrt(running_var + eps)
        out = batch_z * gamma + beta
        ###########################################################################
        #                            END OF YOUR CODE                             #
        ###########################################################################
...
def batchnorm_backward(dout, cache):
    ...
    #############################################################################
    gamma, x, batch_var, batch_z, eps = cache
    
    #out = batch_z * gamma + beta
    #dbeta = dout
    #dgamma = dout * batch_z (dot product)
    #dbatch_z = dout * gamma
    
    dbeta = np.sum(dout, axis=0)
    dgamma = np.sum(dout * batch_z, axis=0)
    dbatch_z = dout * gamma
    
    N = x.shape[0]
    
    dx = (1. / N) * (1. / np.sqrt(batch_var + eps)) * (N * dbatch_z - np.sum(dbatch_z, axis=0) - \
                                                       batch_z * np.sum(dbatch_z * batch_z, axis=0))
      ...
      for j in range(F):
        for out_h in range(out_H):
          for out_w in range(out_W):
            ...
            h_end = h_start + HH
            w_end = w_start + WW
            receptive_field = x_pad[i, :, h_start:h_end, w_start:w_end]
            out[i, j, out_h, out_w] = np.sum(receptive_field * w[j]) + b[j]
        ...
        for j in range(F):
            for h_out in range(out_H):
                for w_out in range(out_W):
                    ...
                    receptive_field = x_pad[i, :, h_start:h_end, w_start:w_end]
                    dw[j] += receptive_field * dout[i, j, h_out, w_out]
                    dx_pad[i, :, h_start:h_end, w_start:w_end] += w[j] * dout[i, j, h_out, w_out]
                    db[j] += dout[i, j, h_out, w_out]
      ...
      for j in range(C):
        for h_out in range(out_H):
          for w_out in range(out_W):
              ...
              dx[i,j,h_start:h_end,w_start:w_end] += mask * dout[i,j,h_out,w_out]
