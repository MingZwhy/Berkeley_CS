...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3 * 32 * 32, hidden_dim=100, num_classes=10,
        ...
        # mean = 0 , standard deviation = weight_scale , shape = (input_dim,hidden_dim)
    ...
    def loss(self, X, y=None):
        ...
        #step1
        layer1_output, cache1 = affine_relu_forward(X,self.params['W1'],self.params['b1'])
        #step2
        layer2_output, cache2 = affine_forward(layer1_output,self.params['W2'],self.params['b2'])
        ...
        #backward gradient
        dlayer1_output, grads['W2'], grads['b2'] = affine_backward(gradient, cache2)
        dx, grads['W1'], grads['b1'] = affine_relu_backward(dlayer1_output, cache1)

        #add L2 gradient to origin gradient
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3 * 32 * 32, num_classes=10,
        ...
        for i in range(self.num_layers):
            layer_name_w = "W{}".format(i+1)
            layer_name_b = "b{}".format(i+1)
            size = (0,0)
            if(i==0):
            ...
            self.params[layer_name_w] = np.random.normal(loc=0, scale=weight_scale, size=size)

            if(i==self.num_layers-1):
                self.params[layer_name_b] = np.zeros(num_classes)
            else:
                self.params[layer_name_b] = np.zeros(hidden_dims[i])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            ...
            if self.use_dropout:
                mask = np.random.rand(*output.shape) > self.dropout_param['p']  # build Dropout mask
                output = output * mask  # apply dropout
